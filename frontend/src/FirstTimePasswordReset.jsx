    ...
    <div style={{minHeight:'100%',background:'linear-gradient(135deg,#0f0f1a 0%,#1a1040 40%,#0f172a 100%)',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:'system-ui,sans-serif'}}>
      <div>
        <div style={{textAlign:'center',marginBottom:28}}>
          <div style={{width:52,height:52,borderRadius:14,background:'rgba(37,99,235,0.6)',border:'1px solid rgba(129,140,248,0.4)',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 12px',fontSize:22}}>✅</div>
        ...
        <div style={{background:'rgba(255,255,255,0.05)',backdropFilter:'blur(24px)',border:'1px solid rgba(255,255,255,0.1)',borderRadius:20,padding:'36px 32px',width:400,boxShadow:'0 32px 80px rgba(0,0,0,0.4)'}}>
          <div style={{fontSize:17,fontWeight:700,color:'#dbeafe',marginBottom:4}}>Welcome, {userName}!</div>
          <div style={{fontSize:13,color:'#6b7280',marginBottom:22}}>Please set a new password to continue.</div>
          <div style={{display:'flex',flexDirection:'column',gap:14,marginBottom:16}}>
            <div>
              <label style={{color:'#93c5fd',fontSize:12,display:'block',marginBottom:4}}>New Password</label>
              <input type="password" value={pw} onChange={e=>setPw(e.target.value)} placeholder="Min 8 chars, uppercase, number, special" style={iS}/>
            </div>
            <div>
              <label style={{color:'#93c5fd',fontSize:12,display:'block',marginBottom:4}}>Confirm Password</label>
          ...
          <button onClick={handleSubmit} disabled={loading} style={{width:'100%',padding:11,borderRadius:9,border:'none',background:'#2563eb',color:'#fff',fontSize:14,fontWeight:600,cursor:loading?'not-allowed':'pointer',opacity:loading?0.8:1}}>
