...
const FORMULA_DEFAULTS = [
  ...
  { section:"Utilization Charts",   accent:"#7c3aed", id:"emp_util_bar",  label:"Employee Utilization % (Bar Chart) — Avg % mode", formula:"Step 1: Collect all tasks logged by the employee in the selected date range. Step 2: Calculate each task's utilization (minutes ÷ daily target × 100). Step 3: Add all those percentages and divide by the number of tasks to get the average. This shows how utilized the employee is per typical work session, making it fair to compare employees regardless of how many tasks they log.", example:"Example: Employee logged 4 tasks — 50%, 60%, 40%, 70% → (50+60+40+70) ÷ 4 = 55% average shown in the bar.", source:"worklog", sourceLabel:"Worklog entries" },
  { section:"Utilization Charts",   accent:"#7c3aed", id:"emp_util_total", label:"Employee Utilization % (Bar Chart) — Total % mode", formula:"Step 1: Collect all tasks logged by the employee in the selected date range. Step 2: Calculate each task's utilization (minutes ÷ daily target × 100). Step 3: Add all those percentages together — no division. This shows the employee's total accumulated effort. An employee with more tasks or longer sessions will naturally show a higher total.", example:"Example: Same 4 tasks — 50%+60%+40%+70% = 220% total shown in the bar.", source:"worklog", sourceLabel:"Worklog entries" },
  { section:"Utilization Charts",   accent:"#7c3aed", id:"dept_util_bar",  label:"Department Utilization % (Bar Chart)", formula:"Same Avg % / Total % toggle applies at department level. In Avg % mode: all task utilizations in the department are summed and divided by the number of tasks. In Total % mode: all task utilizations are simply added up. The department bar compares how departments are performing relative to each other.", example:"Example (Avg): Department has 3 tasks — 60%, 80%, 70% → (60+80+70) ÷ 3 = 70%. Example (Total): 60+80+70 = 210%.", source:"worklog", sourceLabel:"Worklog entries" },
  { section:"Utilization Charts",   accent:"#7c3aed", id:"trend_admin",   label:"Daily Utilization Trend (Admin/Manager view)", formula:"This chart shows the team's average daily utilization over time. Here is how each point on the line is calculated: Step 1 — For each day, look at every employee who logged work. Step 2 — For each employee, add up all their task utilizations for that day to get their personal day total (e.g. two tasks at 50% and 60% = 110% for that employee that day). Step 3 — Average those per-employee day totals across all employees who worked that day. That average becomes the point on the chart. Why can it go above 100%? Because if employees log multiple tasks in a day and their combined minutes exceed the daily target, their day total goes above 100%. The chart reflects real effort, not capped at 100%.", example:"Example: On Apr 10 — Employee A logged 2 tasks: 50%+40% = 90% day total. Employee B logged 2 tasks: 80%+30% = 110% day total. Trend point = (90+110) ÷ 2 = 100%.", source:"worklog", sourceLabel:"Worklog entries" },
];
