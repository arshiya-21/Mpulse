...
const FORMULA_DEFAULTS = [
  ...
  { section:"Utilization Charts",   accent:"#7c3aed", id:"emp_util_bar",  label:"Employee Utilization % (Bar Chart)", formula:"For each employee, all their individual task utilizations in the selected date range are added together and then divided by the number of tasks to get an average. This gives a single % that represents how utilized that employee typically is per work session.", example:"Example: Employee logged 4 tasks with 50%, 60%, 40%, 70% utilization → (50+60+40+70)÷4 = 55% shown in the bar.", source:"worklog", sourceLabel:"Worklog entries" },
  { section:"Utilization Charts",   accent:"#7c3aed", id:"dept_util_bar", label:"Department Utilization % (Bar Chart)", formula:"All tasks logged by employees in a department are collected. Their individual utilization values are added up and divided by the total number of tasks. This gives the department's average utilization per session across the selected date range.", example:"Example: Department has 10 tasks with a total of 600% utilization across them → 600÷10 = 60% for the department.", source:"worklog", sourceLabel:"Worklog entries" },
  { section:"Utilization Charts",   accent:"#7c3aed", id:"trend_admin",   label:"Daily Utilization Trend (Admin/Manager view)", formula:"For each day, each active employee's task utilizations are first summed to get their personal day total. Then those per-employee day totals are averaged across all employees who worked that day. This means a day where 3 people each hit 90% shows as 90%, not inflated by the number of tasks logged.", example:"Example: On Apr 10, Employee A logged 50%+40%=90%, Employee B logged 80%+30%=110% → day trend = (90+110)÷2 = 100%.", source:"worklog", sourceLabel:"Worklog entries" },
];
